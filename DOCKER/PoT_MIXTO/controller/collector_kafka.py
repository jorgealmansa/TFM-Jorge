...
def get_container_id_by_ip(target_ip):
    try:
        container_ids = subprocess.check_output(["sudo", "docker", "ps", "-q"]).decode().strip().splitlines()
        for container_id in container_ids:
            ...
            for net_name, net_data in networks.items():
                container_ip = net_data.get("IPAddress", "")
                if container_ip == target_ip:
                    return container_id
        return None
    except Exception as e:
        logger.exception("Error al obtener el ID del contenedor para la IP %s: %s", target_ip, e)
        return None
...
class IntCollector():
    ...
    def _prepare_reports(self, report):
        # Report structure
        container_id = get_container_id_by_ip(report.address)
        json_report = {
            "measurement": "int_telemetry",
            'time': report.timestamp,
            "fields": {
                "Switch IP": report.address,
                "Container ID": container_id if container_id is not None else "Unknown",
