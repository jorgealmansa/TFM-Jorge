...
python3 parameters_generation/generate_pot_params1.py $1
...
if  [ -n "$2" ] && [ $2 = "docker" ]; then
	...
	if [ $1 = "host" ]; then
		...
		if [ $2 = "1" ]; then
			ip route add 10.1.2.0/24 via 10.1.1.4
		else
...
services:
    nodo1:
        ...
        - NET_ADMIN
        privileged: true
        command: /bin/sh -c \"ip route add 10.1.2.0/24 via 10.0.4.2 && tail -f /dev/null\"
        restart: always
        networks:
            h1_net:
    ...
    nodo2:
        ...
        - NET_ADMIN
        privileged: true
        command: /bin/sh -c \"ip route add 10.1.1.0/24 via 10.0.5.2 && tail -f /dev/null\"
        restart: always
        networks:
            zh2_net:
