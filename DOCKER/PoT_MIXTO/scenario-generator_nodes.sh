...
" > docker/docker-compose-big.yaml
  ...
  for (( i=1; i<=$NUM_MIDDLE_NODES+$NUM_NON_POT_NODES; i++ )); do
    ...
    if [ $i -le $NUM_MIDDLE_NODES ]; then
        ...
        if [[ $i -eq 1 ]]; then
        # middleNode1: comportamiento según número de nodos sin PoT
          if [[ $NUM_NON_POT_NODES -ge 1 ]]; then
            RED_MIDDLE="middle1_middle$(( NUM_MIDDLE_NODES + 1 ))_net"
          else
            RED_MIDDLE="middle1_middle2_net"
          fi
      ...
      # Último nodo con PoT
      if [[ $NUM_MIDDLE_NODES -eq 2 && $NUM_NON_POT_NODES -gt 1 ]]; then
        SUF=$(( NUM_MIDDLE_NODES + NUM_NON_POT_NODES + 1 ))
        RED1="middle${i}_middle$(( NUM_MIDDLE_NODES + NUM_NON_POT_NODES ))_net"
        RED1_IP="10.0.${SUF}.2"
        RED2="zegress_net"
        RED2_IP="10.0.$(( i + 1 )).3"
        EXTRA_CMD=""
      elif [[ $NUM_NON_POT_NODES -eq 1 && $NUM_MIDDLE_NODES -eq 2 ]]; then
        RED1="middle$(( i ))_middle$(( i + 1))_net"
        RED1_IP="10.0.$((i + NUM_NON_POT_NODES + 1)).2"
        RED2="zegress_net"
        RED2_IP="10.0.$(( i + 1 )).3"
        EXTRA_CMD="script"
      elif [[ $NUM_NON_POT_NODES -eq 0 ]]; then
        RED1="middle$(( i-1 ))_middle${i}_net"
        RED1_IP="10.0.${i}.2"
        RED2="zegress_net"
        RED2_IP="10.0.$(( i + 1 )).3"
        EXTRA_CMD=""
      ...
      echo "  middleNode$i:
        ...
        cap_add:
          - NET_ADMIN
        privileged: true
        command: /home/p4/pot/docker/run.sh $(( i + 11 )) $EXTRA_CMD
        networks:
          $RED1:
            ipv4_address: $RED1_IP
          $RED2:
            ipv4_address: $RED2_IP
          controller:
            ipv4_address: 10.0.0.$(( i + 11 ))
        volumes:
          - ../:/home/p4/pot
    ...
    elif [[ $i -eq 2 && $NUM_MIDDLE_NODES -gt 2 ]]; then
      if [[ $NUM_NON_POT_NODES -gt 0 ]]; then
        SUF1=$(( NUM_MIDDLE_NODES + NUM_NON_POT_NODES + 1 ))
        RED1="middle${i}_middle$((NUM_MIDDLE_NODES + NUM_NON_POT_NODES))_net"
        RED1_IP="10.0.$SUF1.2"
        RED2="middle${i}_middle$((i + 1))_net"
        RED2_IP="10.0.$((i + 1)).3"
      else
        RED1="middle$((i-1))_middle${i}_net"
        RED1_IP="10.0.${i}.2"
