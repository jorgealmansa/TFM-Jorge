...
class SwitchConnection(object):
    ...
    # The _init_ method is the only modification. The older version of the function taken from the P4 tutorials repository is commented
    ...
    def __init__(self, name=None, address='127.0.0.1:50051', device_id=0,
                 proto_dump_file=None, channel=None, cacert=None, cert= None, private_key=None):
        self.name = name
        self.address = address
        self.device_id = device_id
        self.p4info = None
        if channel is not None:
            ...
            if cacert is not None:
                root_certificates = read_pem_file(cacert)
            certificate_chain = None
            if cert is not None:
                certificate_chain = read_pem_file(cert)
            privatekey = None
            if private_key is not None:
                privatekey = read_pem_file(private_key)
            creds = grpc.ssl_channel_credentials(root_certificates, privatekey, certificate_chain)
            try:
                self.channel = grpc.secure_channel(self.address, creds)
            except Exception:
                print("Failed to connect to P4Runtime server")
        else: 
            self.channel = grpc.insecure_channel(self.address)
        if proto_dump_file is not None:
            interceptor = GrpcRequestLogger(proto_dump_file)
...
class GrpcRequestLogger(grpc.UnaryUnaryClientInterceptor,
    ...
    def SetForwardingPipelineConfig(self, request, context):  
        ...
        device_config = request.config.p4_device_config  # bytes del JSON  
        if not device_config:  
            # Devolver error si la config está vacía  
            context.set_code(grpc.StatusCode.INVALID_ARGUMENT)  
            context.set_details("Empty device config")  
            return p4runtime_pb2.SetForwardingPipelineConfigResponse()  
        # Aplicar el JSON al switch BMv2  
        try:  
            with open('/tmp/p4prog.json', 'wb') as f:  
                f.write(device_config)  
            bm_result = apply_bmv2_json('/tmp/p4prog.json')  # pseudocódigo  
        except Exception as e:  
            context.set_code(grpc.StatusCode.INTERNAL)  
            context.set_details(str(e))  
            return p4runtime_pb2.SetForwardingPipelineConfigResponse()  
    # Guardar P4Info para consultas posteriores  
    self.p4info = p4info  
        return p4runtime_pb2.SetForwardingPipelineConfigResponse()  
        def GetForwardingPipelineConfig(self, request, context):
        # Suponiendo que self.p4info se guardó previamente
        rep = p4runtime_pb2.GetForwardingPipelineConfigResponse()
        if request.response_type == p4runtime_pb2.GetForwardingPipelineConfigRequest.P4INFO_AND_COOKIE:
            rep.config.p4info.CopyFrom(self.p4info)
            # Cookie no implementada en este caso, se deja por defecto (cero)
        elif request.response_type == p4runtime_pb2.GetForwardingPipelineConfigRequest.COOKIE_ONLY:
            # Devolver solo cookie si se pidiera (no es el caso típico aquí)
            pass
        return rep
